...
let onSessionExpired: (() => void | Promise<void>) | undefined;

export class ApiError extends Error {
  constructor(readonly status: number, message: string, readonly body?: unknown) { super(message); this.name = "ApiError"; }
}

export function setSessionExpiredHandler(handler: (() => void | Promise<void>) | undefined) { onSessionExpired = handler; }
export function getSession() { return session; }
function isSession(value: unknown): value is Session {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Partial<Session> & { version?: unknown };
  return (candidate.version === undefined || candidate.version === 1 || candidate.version === SESSION_VERSION)
    ...
    && candidate.token_type === "bearer" && typeof candidate.expires_in === "number"
    && !!candidate.user && typeof candidate.user.id === "number" && typeof candidate.user.email === "string";
}
export async function loadStoredSession(): Promise<Session | null> {
  const stored = await SecureStore.getItemAsync(SESSION_KEY);
  if (!stored) { session = null; return null; }
  try {
    const parsed: unknown = JSON.parse(stored);
    if (!isSession(parsed)) { await clearSession(); return null; }
    session = { ...parsed, version: SESSION_VERSION };
    if ((parsed as { version?: number }).version !== SESSION_VERSION) await persistSession();
    return session;
  } catch { await clearSession(); return null; }
}
async function persistSession() { if (session) await SecureStore.setItemAsync(SESSION_KEY, JSON.stringify(session)); }
export async function saveSession(value: Session) { session = { ...value, version: SESSION_VERSION }; await persistSession(); }
export async function clearSession() {
  session = null;
  try { await SecureStore.deleteItemAsync(SESSION_KEY); } catch { /* local state is authoritative when storage is unavailable */ }
}
export async function loadLastLoginEmail(): Promise<string> { return (await SecureStore.getItemAsync(LAST_LOGIN_EMAIL_KEY)) ?? ""; }
export async function saveLastLoginEmail(email: string) { await SecureStore.setItemAsync(LAST_LOGIN_EMAIL_KEY, email.trim().toLowerCase()); }

async function refresh(): Promise<boolean> {
  if (!session?.refresh_token) return false;
  if (!refreshPromise) {
    const refreshToken = session.refresh_token;
    refreshPromise = fetch(`${API_BASE_URL}/auth/refresh`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refresh_token: refreshToken }) })
      .then(async response => {
        if (!response.ok) return false;
        const tokens = await response.json() as Pick<Session, "access_token" | "refresh_token" | "token_type" | "expires_in">;
        if (!tokens.access_token || !tokens.refresh_token || tokens.token_type !== "bearer") return false;
        await saveSession({ ...session!, ...tokens });
        return true;
      }).catch(() => false).finally(() => { refreshPromise = null; });
  }
  const refreshed = await refreshPromise;
  if (!refreshed) { await clearSession(); await onSessionExpired?.(); }
  return refreshed;
}
...
export const api = {
  ...
  transactions: (filters: MovementFilters = {}) => { const params = new URLSearchParams({ page: String(filters.page ?? 1), page_size: String(filters.page_size ?? 50) }); Object.entries(filters).forEach(([key, value]) => { if (key !== "page" && key !== "page_size" && value !== undefined) params.set(key, String(value)); }); return request<TransactionPage>(`/transactions?${params.toString()}`); },
