...
import { commonStyles } from "../../src/ui/theme";
...
export default function MovementDetail() {
  ...
  return <ScrollableScreen contentContainerStyle={styles.page}><Text style={styles.title}>Editar movimiento</Text><Text style={styles.muted}>{item.kind === "transfer" ? "Transferencia" : item.kind === "income" ? "Ingreso" : "Gasto"}</Text>
    ...
    <Text style={styles.muted}>Saldo y conversión calculados por el backend. Actual: {formatMoney(item.amount, currency)}</Text><Button title={update.isPending ? "Guardando…" : "Guardar cambios"} disabled={update.isPending || item.is_voided} onPress={() => update.mutate()} /><View style={styles.danger}><Button title={item.is_voided ? "Movimiento anulado" : "Anular movimiento"} disabled={item.is_voided || voidMutation.isPending} color="#b91c1c" onPress={() => Alert.alert("¿Anular este movimiento?", "El historial se conservará.", [{ text: "Cancelar", style: "cancel" }, { text: "Anular", style: "destructive", onPress: () => voidMutation.mutate() }])} /></View></ScrollableScreen>;
}
