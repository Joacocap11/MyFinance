...
export default function MovementDetail() {
  ...
  const update = useMutation({
    ...
    onSuccess: async () => { await Promise.all(["transactions", "dashboard", "accounts", "transaction"].map(key => client.invalidateQueries({ queryKey: [key] }))); router.back(); },
    onError: error => Alert.alert("No se pudo editar", error instanceof ApiError ? error.message : "Revisá los datos ingresados."),
  });
  const voidMutation = useMutation({ mutationFn: () => api.voidTransaction(transactionId), onSuccess: async () => { await Promise.all(["transactions", "dashboard", "accounts"].map(key => client.invalidateQueries({ queryKey: [key] }))); router.back(); }, onError: error => Alert.alert("No se pudo anular", error instanceof ApiError ? error.message : "Revisá los datos ingresados.") });
